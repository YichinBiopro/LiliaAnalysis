...
custom marker 模式已支援缺口分段濾波，並以指定 `--fs` 對齊品質與 qEEG 視窗。沒有窗口 metadata 的舊 entropy CSV 仍要求原始錄製連續。新版 CSV 連同 `.csv.meta.json` 可在核對來源、窗口索引與 timestamps 後，繪製有缺口的 session、event 及 absolute 圖；保留缺口後第一個有效點，平滑與連線均不跨區段。搬移新版 CSV 時須一併保留 sidecar。
