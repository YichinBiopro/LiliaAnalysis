"""Versioned band-entropy tables with verifiable sample/time alignment."""
...
def write_entropy_table(path, source, frame, grid, parameters, code_sha256):
    ...
    try:
        ...
        metadata = {
            'kind': 'band_entropy', 'schema_version': SCHEMA_VERSION,
...
def load_entropy_table(path, raw_csv=None, channel=None):
    ...
    if not required.issubset(frame) or not sidecar.exists() or frame.empty:
        raise ValueError('Incomplete band-entropy window metadata; regenerate the analysis CSV')
    metadata = json.loads(sidecar.read_text(encoding='utf-8'))
    parameters = metadata.get('parameters', {})
    if (metadata.get('kind') != 'band_entropy' or metadata.get('schema_version') != SCHEMA_VERSION
            or metadata.get('config_id') != config_id(parameters)
            or metadata.get('table_sha256') != file_sha256(path)):
        raise ValueError('Band-entropy table or configuration fingerprint mismatch')
    for key, expected in [('window_schema_version', SCHEMA_VERSION),
        ...
        if not (frame[key] == expected).all():
            raise ValueError(f'Inconsistent band-entropy {key}')
    if channel is not None and channel != parameters.get('channel'):
        raise ValueError('Requested channel differs from the band-entropy table')
    if raw_csv is not None:
        if file_sha256(raw_csv) != metadata['source_id']:
            raise ValueError('Raw recording differs from the band-entropy source')
        ...
        if (len(t) != metadata['source_samples'] or len(frame) != len(grid.starts)
            ...
            raise ValueError('Raw recording and band-entropy window counts differ')
        for key, expected in grid.columns.items():
            ...
            if not valid:
                raise ValueError(f'Band-entropy {key} does not match raw timestamps')
