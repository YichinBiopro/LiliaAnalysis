...
Entropy 品質診斷增量以 `tools/validate_entropy_quality_state_stage18.py --diagnostics --out /path/to/new-directory` 驗證。窗口 CSV 四診斷欄位對應 sidecar `quality_audit`；state summary 記診斷計數，逐窗紀錄在 states audit。來源 reader 按實際 raw／filtered 重建品質輸入，clean 另核對 raw precheck；denoised MI 仍關閉品質評分且不重跑模型。新 summary reader 另綁定未遮罩 population 與 series scope。數值、例外可驗證範圍及五圖目視見 [entropy 增量報告](STAGE18_ENTROPY_DIAGNOSTICS_REPORT_2026-09-15.md)。

Goertzel 品質增量以 `tools/validate_goertzel_quality_stage18.py --out /path/to/new-directory` 驗證。`goertzel` reader 重建 float32 分段 BP、整數時間／窗口、Goertzel power、raw sat／PTP／max diff、filtered edge shift 及 filtered 全通道品質診斷；CSV 仍取指定通道 legacy overall。cache 重繪另保留舊 fingerprint guard，來源 reader 不要求當前 code hash 才能回讀歷史證據。舊完整表無整組診斷仍可讀，external 與歷史例外限制沿用共用 validator；不得把它們當作可完全重算結果。19 案例及污染 helper／七圖目視見 [Goertzel 增量報告](STAGE18_GOERTZEL_QUALITY_REPORT_2026-09-15.md)。

R6 quality_check 以 `tools/validate_quality_check_stage18.py --out /path/to/new-directory` 驗證。`quality_check_samples` reader 重建來源 float32 raw／filtered 片段、抽樣起點、分數與兩 stage 逐窗診斷；`quality_check_anomalies` reader 重建 raw 10 秒窗口、clip／flat／gap、qmed、原因與 severity。兩表 sidecar 保存來源 hash、設定與 `quality_diagnostics_version=1` audit；外部 scorer 診斷明示 unavailable，歷史 component 例外僅核對保存 fallback 與可重算部分。NaN qmed 另 flagged 為 `invalid-Q non-finite`，有限分數及舊門檻政策不改；20 案例及污染 helper、八圖目視見 [R6 增量報告](STAGE18_QUALITY_CHECK_REPORT_2026-09-15.md)。

第十八階段 direct helper 以 `tools/validate_direct_quality_helper_stage18.py --out /path/to/new-directory` 凍結舊版 `plot_event_markers.py`／`plot_index_vs_raw.py`，比較 `compute_quality_windowed` 與 `plot_custom_markers` 的舊新數值。`custom_marker_quality` reader 重建 raw 單通道品質窗口、整數時間、qEEG 窗口／缺口 mask、舊 score 門檻與四診斷欄位；外部舊 scorer 僅驗證保存的分數／audit 一致，診斷明示 unavailable。`return_audit=False` 保留雙值 API，缺口窗口分數仍保留但品質線斷開。整階段 8 組證據沿用原 hash／NPZ，較早被後續修改的程式 hash 明列排除；詳見 [完整驗收](STAGE18_ACCEPTANCE_REPORT_2026-09-15.md)。
